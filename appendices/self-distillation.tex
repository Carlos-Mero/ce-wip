...
\begin{table}[htbp]
\centering
\caption{Evaluation results of OPD and SFT with 16 sampled responses per problem. We can see that OPD typically outperforms SFT in proximal distillation, while it may experience severe instability issue and performance degradation in distal distillation. Proximal teachers are obtained by training the base model with GRPO for 50, 100, or 200 steps.}
...
%   \caption{Additional results on the training dynamics of different self-distillation variants. We can see that neither iterative self-OPD nor proximal SFT experienced entropy collapse. These experiments were conducted on Qwen3-1.7B-Base with a learning rate of $10^{-5}$ with AdamW optimizer.}
%   \label{fig:distillation-proximal-limit}
% \end{figure}
\subsection{Supplimentary Experiments on Standard and Reversed Iterative Self-Distillation}\label{app:supp-exp-isd}
...
In a single training step, model entropy may randomly increase or decrease, but over the long run it exhibits a deterministic downward trend in expectation. These entropy curves satisfies the derivation in the main body very well. In fact the decreasing expected entropy does not guarantee the probability of decrement and increment. For example in reversed iterative self-distillation, since the punishment on samples with a high probability will raise the entropy, the entropy policy model has a high probability to be increased. However, the negative expectation indicates that once the decrement happens, the magnitude of the decrease will be substantial. This was also observed in our experiments.
