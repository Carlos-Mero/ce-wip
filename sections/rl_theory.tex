...
For a prompt $x$ with deterministic binary rewards, write $\rho=\rho_\theta(x)\in(0,1)$ for the accuracy and suppress conditioning on $x$. Let $H(\pi_\theta^+)$ and $H(\pi_\theta^-)$ denote the true (success-conditioned) and false (failure-conditioned) entropies, respectively. Then
\begin{equation}
