...
\end{equation}
Thus a detached reward $-\beta\widehat c_i$, with $\beta\ge0$, gives an expected policy-gradient contribution proportional to the negative conditional KL gradient. The signal is invariant to adding a common constant to all successful responses' log-ratios.
\end{theorem}
...
This advantage of CKL still holds for up to 20 training epochs, as shown in Figure~\ref{fig:ext-ckl-cmp}. We then evaluated these models on different math problem solving datasets, where CKL regularization did exhibit higher performance and generalizability over KL regularization, See Figure~\ref{fig:passk-eval}. More related results could be found in Figure~\ref{fig:passk-eval-full}.
